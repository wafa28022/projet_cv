...
class DistractionDataset(Dataset):
    def __init__(self, video_path, annotations_csv, session_name,
        ...
        """
        Args:
            video_path      : chemin vers le mosaic .avi
            annotations_csv : chemin vers le CSV des labels
            session_name    : 's1', 's4', etc.
            img_size        : (224, 224)
            seq_len         : nombre de frames par séquence
            sample_every    : 1 frame toutes les N frames
            augment         : True pour train
        """
    ...
    def __getitem__(self, idx):
        """
        Retourne :
            body_seq  : [seq_len, 3, 224, 224]
            face_seq  : [seq_len, 3, 224, 224]
            hands_seq : [seq_len, 3, 224, 224]
            label     : tensor int
        """
